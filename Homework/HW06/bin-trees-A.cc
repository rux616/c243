...
int main()
{
    ...
    do  // until interactive user signals to quit
    {
        ...
        switch (response)  // Response is necessarily one of the following.
        {
            ...
            case 'Q':
            case 'q':
                break; // Do nothing.  The program will quit.
            case 'R':
            case 'r':
                cout << endl;
                reverse(treep);
...
void printMenuAndGetAction(char & response)
{
    ...
    cout << "   C)ount and report number of zero data values in current tree\n";
...
{
    if (!rootp)
        //Base case.  If the node doesn't exist, then it won't have a zero value.
        return 0;
    else
        //If the node exists, check whether its datum is zero.  If it is, add 1 to the return value.
        //If it isn't, add 0 to the return value.  The return value is also a recursive call that
        //evaluates both subtrees of the node specified by rootp.
        return   (rootp->datum ? 0 : 1)
...
{
    if (!rootp)
        //Base case.  If the node doesn't exist, return -1.
        return -1;
    else
    {
        //Declare two holding variables to avoid exponential insanity.
        int heightLeft = height(rootp->left);
        int heightRight = height(rootp->right);
        //If the node exists, then return 1 plus the greater of the heights of the left or right
        //subtree.
...
void incrementEachDatum(nodePtr rootp)
{
    //Only increment nodes which exist.
    if (rootp)
    {
        //Increment the datum of the node.
        ++(rootp->datum);
        //Recursively increment the left and right subtrees.
...
void reverse(nodePtr &rootp)
{
    //Only attempt to reverse an existing node.
    if (rootp)
    {
        //Swap the values of the left and right attributes.
...
{
    if (!rootp)
        //Base case.  If a node doesn't exist, it's not going to have a datum to sum.  Return 0.
        return 0;
    else
        //If the datum is negative, returns that plus the results of a recursive call to the left
        //and right subtrees, otherwise returns 0 plus the results of the recursive calls.
        return   (rootp->datum < 0 ? rootp->datum : 0)
               + sumOfNegativeData(rootp->left)
